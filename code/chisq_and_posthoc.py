...
#Source: https://github.com/neuhofmo/chisq_test_wrapper/blob/master/chisq_test_wrapper.py
#Explanation: https://neuhofmo.github.io/chi-square-and-post-hoc-in-python/
...
def run_chisq_on_each_row(df,rowLabel):
    df['Total'] = df.sum(axis=1)
    new_df = df[(df.index==rowLabel) | (df.index=='Total')]
    chi2, p, dof, ex = chi2_contingency(new_df, correction=True)
    return p
...
def chisq_and_posthoc_corrected(df, correction_method='fdr_bh', alpha=0.05):
    ...
    p_vals = [run_chisq_on_each_row(df, row) for row in list(df.index)]

    # correction for multiple testing
    reject_list, corrected_p_vals = multipletests(p_vals, method=correction_method, alpha=alpha)[:2]
    for p_val, corr_p_val, reject, comb in zip(p_vals, corrected_p_vals, reject_list, list(df.index)):
